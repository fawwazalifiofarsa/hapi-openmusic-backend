...
class SongsService {
  ...
  async addSong({
    ...
    // Get an existing album id from the albums table
    const albumQuery = 'SELECT id FROM albums ORDER BY RANDOM() LIMIT 1';
    const albumResult = await this._pool.query(albumQuery);

    if (albumResult.rows.length === 0) {
      throw new InvariantError('No existing albums found');
    }

    // Use the retrieved album id as albumId
